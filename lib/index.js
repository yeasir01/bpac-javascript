...
export class BrotherSdk {
     ...
     * @returns {Promise<boolean>} A Promise that resolves with the result of the document opening operation
     ...
     * Closes the document.
     *
     * @returns {Promise<boolean>} A Promise that resolves with a boolean of the closing operation
     *                        or rejects with an error message if there is a failure.
     *
     * @throws {Error} Throws an error with a descriptive message if unable to close the document.
     *
     */
    close() {
        const eventName = "IDocument::Close";
        ...
        const promise = new Promise((resolve, reject) => {
            const handler = (e) => {
                ...
                if (e.detail.connect === false) {
                    reject("Unable to close the document.");
                } else {
                    resolve(e.detail.ret);
                }
            };
    ...
    /**
     * Retrieves image data with the specified width and height parameters.
     *
     * @param {number} [width=0] - The width of the image. If set to 0, it will be set to auto.
     * @param {number} [height=0] - The height of the image. If set to 0, it will be set to auto.
     * @returns {Promise<any>} A Promise that resolves with the retrieved image data in base64
     *                        or rejects with an error message if there is a failure.
     *
     * @throws {Error} Throws an error with a descriptive message if unable to retrieve
     *                  image data.
     *
     */
    getImageData(width = 0, height = 0) {
        const eventName = "IDocument::GetImageData";
        const message = { method: eventName, type: 4, width, height };

        const data = new Promise((resolve, reject) => {
            const handler = (e) => {
                document.removeEventListener(eventName, handler);

                if (e.detail.ret && e.detail.connect) {
                    resolve(e.detail.image);
                } else {
                    reject("Unable to retrieve image data.");
                }
            };
            document.addEventListener(eventName, handler);
        });
